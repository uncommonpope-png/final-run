...
class NLCommandRouter {
    ...
    async route(input) {
        ...
        if (matches.length > 0) {
            ...
            console.log(`[NL] "${input.substring(0, 50)}..." → ${best.skill}`);
        ...
        if (this.brain) {
            try {
                const llmResult = await this._llmClassify(input);
                if (llmResult && llmResult.skill) {
                    console.log(`[NL] "${input.substring(0, 50)}..." → ${llmResult.skill} (via LLM)`);
                    return await this._execute(llmResult.skill, llmResult.params || {});
                }
            } catch (e) {
                console.log(`[NL] LLM classification failed: ${e.message}`);
            }
        }
        ...
        return {
            status: 'unrecognized',
            input,
            message: `I understood your input but couldn't map it to a skill. Here's what I can do: ${this.skillMap.map(s => s.skill).join(', ')}`,
            suggestions: this.skillMap.map(s => s.skill),
        };
    }
