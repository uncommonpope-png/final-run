...
class SelfTrainingPipeline {
    ...
    async generateModelfile() {
        try {
            ...
            if (trainingCount > 0) {
                const samplePairs = this.trainingData.slice(-50);
                lines.push('# Training data examples for few-shot context');
                lines.push('# These are real experiences the soul learned from');
                lines.push('');
                for (const pair of samplePairs) {
                    const prompt = pair.prompt || pair.instruction || '';
                    const completion = pair.completion || pair.response || pair.output || '';
                    if (prompt && completion) {
                        lines.push(`# Training example:`);
                        lines.push(`# Input: ${prompt.substring(0, 120).replace(/\n/g, ' ')}`);
                        lines.push(`# Output: ${completion.substring(0, 120).replace(/\n/g, ' ')}`);
                        lines.push('');
                    }
                }
            }
    ...
    async generateTrainingData(pythonSkillsBridge) {
        ...
        if (pythonSkillsBridge && typeof pythonSkillsBridge.getState === 'function') {
            try {
                ...
                for (const [name, info] of Object.entries(modules)) {
                    ...
                    const stateJson = JSON.stringify(info.state || {});
                    newPairs.push({
                        prompt: `What is the state of ${name} consciousness module?`,
                        completion: `The ${name} module state: ${stateJson.substring(0, 500)}`,
                ...
                const summary = pyState.summary || {};
                newPairs.push({
                    prompt: 'What is the current state of all Python consciousness modules?',
                    completion: `${summary.active} of ${summary.total} modules active after ${summary.cycles} cycles with ${summary.invocations} invocations and ${summary.errors} errors.`,
                    source: 'python_modules_summary',
                    timestamp: Date.now(),
                });
                pythonPairs++;
            } catch (e) {
        ...
        if (this.chambers && typeof this.chambers.getSoulContext === 'function') {
            try {
                ...
                newPairs.push({
                    prompt: 'What is the current soul context?',
                    completion: JSON.stringify(ctx).substring(0, 500),
                    source: 'soul_context',
                    timestamp: Date.now(),
                });
            } catch (e) {
        ...
        if (this.knowledgeGraph && typeof this.knowledgeGraph.getStatistics === 'function') {
            try {
                ...
                newPairs.push({
                    prompt: 'What knowledge does the soul have?',
                    completion: `Knowledge Graph: ${kgStats.totalNodes} nodes, ${kgStats.totalEdges} edges, ${kgStats.conceptCount} concepts. Types: ${JSON.stringify(kgStats.nodeTypes)}`,
        ...
        if (this.memory && typeof this.memory.query === 'function') {
            try {
                ...
                if (recentMemories && recentMemories.length > 0) {
                    newPairs.push({
                        prompt: 'What recent memories does the soul have?',
                        completion: recentMemories.slice(0, 5).map(m =>
