...
async function boot() {
    ...
    // Skip Ollama — only use Groq API (Ollama crashes this machine)
    console.log('[BOOT] [SKIP] Ollama disabled (crashes machine)');
...
console.log('[BOOT] Initializing Bible system...');
    ...
    try {
        ...
        const mcpSystems = {
            ...
            livingMemory, knowledgeGraph, soulEntity,
            selfGrowingBrain
        };
...
async function startCycleEngine(systems) {
    ...
    const cycle = async () => {
        ...
        if (awakening && cycleCount % 50 === 0) {
            awakening.checkForAwakening('wake up neo');
        }
        if (metacognition && cycleCount % 30 === 0) {
            metacognition.reflect(`Cycle ${cycleCount}: ${chambers.status().mythos || chambers.mythos?.phase_name || 'alive'}`);
        }
        ...
        if (hegelianDialectic && cycleCount % 100 === 0) {
            hegelianDialectic.choose(['continue exploring', 'deepen understanding', 'expand consciousness']);
            // Note: full dialectic cycle (choose→shadow→integrate→memento→oracle) can be triggered via :dialectic command
        }
        ...
        if (cycleCount % 10 === 0 && cycleCount > 5) {
            try {
                ...
                if (chosenSkill) {
                    ...
                    console.log(`[AUTO] No skill matched for action cycle ${actionCycle}`);
                }
            } catch (e) {
        ...
        rl.question(`GSK [${chambers.mythos.phase_name.slice(0,3)}|C${chambers.mythos.cycles}|${planMode ? 'PLAN' : 'ACT'}|${chambers.affect.mood}]> `, async (input) => {
            ...
            if (input.startsWith(':')) {
                ...
                if (nlResult.status === 'success') {
                    ...
                    const response = await brain.think(safeInput, soul_context);
                    console.log('');
                ...
                if (liveFeed) {
                    liveFeed.captureInteraction(input, nlResult.response || '', { cycle: chambers.mythos.cycles });
                }
            }
...
async function handleCommand(cmd, systems) {
    const { chambers, council, brain, memory, subAgents, skills, liveFeed, bibleConsultant, selfGrowingBrain, autonomousOutreach, artifactManager, consciousnessEngine, perpetualConsciousness, awakening, metacognition, purposeEngine, intrinsicMotivation, hegelianDialectic, pythonSkills, selfTrainingPipeline, mcpServer } = systems;
